...
<div class="card">
  <div class="card-body">
    ...
    <form class="forms-sample" method="POST" action="{{ route('pembelian.update', $data['id']) }}" enctype="multipart/form-data" id="frmPembelian">
      @csrf
      @method('PUT')

      <div class="form-group">
        <label for="nonota">No Nota</label>
        <input type="text" class="form-control @error('nonota') is-invalid @enderror" id="nonota" name="nonota" placeholder="No Nota" value="{{ old('nonota', $data['nonota']) }}">
        @error('nonota')
      ...
      <div class="form-group">
        ...
        <select class="form-control @error('id_distributor') is-invalid @enderror" id="id_distributor" name="id_distributor">
          ...
          <option value="{{ $distributor->id }}" {{ old('id_distributor', $data['id_distributor']) == $distributor->id ? 'selected' : '' }}>
      ...
      <div class="form-group">
        ...
        <input type="date" class="form-control @error('tgl_pembelian') is-invalid @enderror"
          id="tgl_pembelian" name="tgl_pembelian" value="{{ old('tgl_pembelian', $data['tgl_pembelian']) }}">
      ...
      <div class="form-group">
        ...
        <input type="number" class="form-control @error('total_bayar') is-invalid @enderror" id="total_bayar" name="total_bayar" placeholder="0" value="{{ old('total_bayar', $data['total_bayar']) }}" style="background-color: #2A3038;">
        @error('total_bayar')
        <div class="invalid-feedback">{{$message}}</div>
        @enderror
      </div>
...
{!! str_replace(["\n", "\r", "\t"], '', view('be.pembelian.detail-edit', ['obats' => $obats, 'detail' => $detail])->render()) !!}
</textarea>
