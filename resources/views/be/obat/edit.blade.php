...
<div class="card">
    <div class="card-body">
        ...
        <form class="forms-sample" method="POST" action="{{ route('obat.update', $data['id']) }}" enctype="multipart/form-data" id="frmObat">
            @csrf
            @method('PUT')

            <div class="form-group">
                <label>Nama Obat</label>
                <input type="text" class="form-control @error('nama_obat') is-invalid @enderror" id="nama_obat" name="nama_obat" value="{{ old('nama_obat', $data->nama_obat) }}" placeholder="Nama Obat">
                @error('nama_obat')
            ...
            <div class="form-group">
                ...
                <select class="form-control @error('id_jenis_obat') is-invalid @enderror" id="jenis_obat" name="id_jenis_obat">
                    <option value="">-- Pilih Jenis Obat --</option>
                    @foreach ($jenis_obat as $jo)
                        <option value="{{ $jo->id }}" {{ (old('id_jenis_obat', $data->id_jenis_obat) == $jo->id) ? 'selected' : '' }}>
                            {{ $jo->jenis }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input style="background-color: #2A3038;" type="number" class="form-control @error('harga_jual') is-invalid @enderror" id="harga_jual" name="harga_jual" value="{{ old('harga_jual', $data->harga_jual) }}" placeholder="Harga Jual" @if(Auth::user()->jabatan === 'karyawan') disabled @endif>
                @error('harga_jual')
                    <div class="invalid-feedback">{{$message}}</div>
                @enderror
            </div>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control @error('deskripsi_obat') is-invalid @enderror" id="deskripsi_obat" name="deskripsi_obat" rows="4">{{ old('deskripsi_obat', $data->deskripsi_obat) }}</textarea>
            ...
            <div class="form-group">
                <label>Stok</label>
                <input type="number" class="form-control @error('stok') is-invalid @enderror" id="stok" name="stok" value="{{ old('stok', $data->stok) }}" placeholder="Stok">
                @error('stok')
                    <div class="invalid-feedback">{{$message}}</div>
                @enderror
            </div>
            ...
            <a href="{{ route('obat.index') }}" class="btn btn-dark">Cancel</a>
        </form>
    </div>
</div>
